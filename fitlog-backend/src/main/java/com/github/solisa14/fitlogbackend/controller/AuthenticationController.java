...
public class AuthenticationController {
     ...
     * @return ResponseEntity containing the authentication response with JWT token and refresh
     *         token
     */
    @PostMapping("/login")
    public ResponseEntity<AuthenticationResponseDto> login(
        ...
        // If authentication is successful, retrieve user details and generate JWT token and refresh
        // token
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .body(new AuthenticationResponseDto(userDetails.getUsername(), token,
                        jwtUtil.getExpirationTime(), refreshToken.getToken()));
    }
     ...
     * 
     * @param request a map containing the refresh token
     * @return ResponseEntity containing the new authentication response with access token and
     *         refresh token
     */
    @PostMapping("/refresh")
    public ResponseEntity<AuthenticationResponseDto> refresh(
        ...
        return refreshTokenService.findByToken(refreshToken).filter(token -> !token.isExpired())
                .map(token -> {
                    String newAccessToken = jwtUtil.generateToken(token.getUser());
                    return ResponseEntity
                            .ok(new AuthenticationResponseDto(token.getUser().getUsername(),
                                    newAccessToken, jwtUtil.getExpirationTime(), refreshToken));
